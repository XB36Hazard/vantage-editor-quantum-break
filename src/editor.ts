...
export class Editor implements SaveEditor 
{
    ...
    private Current_TimelineChanged(newValue, oldValue) {
    ...
    {
        this.QBS = new Quantum_Break_Save(new Stream(buffer));    
        if(this.QBS.IsValid)
        {
            this.Current_Timeline = this.QBS.Time_Line;
            this.Update();
        }
    }
    public Update()
    {
        if(this.Current_Timeline_2 !== this.Current_Timeline)
        {
            if(this.QBS !== null)
            {
                if(this.QBS.IsValid)
